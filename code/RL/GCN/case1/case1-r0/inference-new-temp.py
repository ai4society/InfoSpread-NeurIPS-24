...
if __name__ == "__main__":
    datasets_path = '/Users/bittu/Desktop/InfoSpread-server/dataset/generate_dataset/deg_dataset_new/'
    ...
    for dataset_path in sorted(glob(datasets_path + f'{args.nodes}/*.pkl')):
        ...
        if str(args.nodes) in dataset_path:
            for model in sorted(glob(models_path + '*.pt')):
                values = []
                if 'target' in model and 'mn'+str(10) in model and 'model_1_1' in model:
        ...
        df.to_csv(f'output/inference/10GCN50{dataset_name}_inf_res.csv', index=False)
